...
from flight_control import AdvisoryFlightControl, ContingencyAction, ContingencyCommand, FlightControl
...
class SensorSentry:
    ...
    def __init__(self, vehicle_id: str, hardware_bus: HardwareBus,
                 flight_control: Optional[FlightControl] = None,
                 aircraft_mode: bool = True,
                 ml_weights_path: Optional[str] = None):
        self.vehicle_id = vehicle_id
        self.bus = hardware_bus
        # Aircraft actions are deliberately separate from the generic hardware
        # bus.  A drone must not receive the ground-vehicle "motor kill" signal.
        self.flight_control = flight_control or AdvisoryFlightControl()
        self.aircraft_mode = aircraft_mode
        self.last_contingency_acknowledged = False
        ...
        self.ml_monitor = LSTMAutoEncoderMonitor(weights_path=ml_weights_path)
    ...
    def _request_contingency(self, timestamp: float, action: ContingencyAction,
                             reason: str) -> bool:
        """Record and request an aircraft-safe contingency action.

        The bus message is an audit/telemetry event.  Actual flight control is
        performed only by the explicitly configured FlightControl adapter, and
        an unacknowledged request is never represented as a completed action.
        """
        command = ContingencyCommand(timestamp, action, reason)
        self.bus.send(command.to_bytes(), 'CONTINGENCY')
        self.last_contingency_acknowledged = self.flight_control.request(action, reason)
        return self.last_contingency_acknowledged

    def _protective_stop(self, timestamp: float, attack_type: str, reason: str) -> None:
        """Use an aircraft contingency, or retain brake semantics for ground robots."""
        if self.aircraft_mode:
            self._request_contingency(timestamp, ContingencyAction.HOLD, reason)
            return
        stop_sig = self.safe_stop.trigger_safe_stop(timestamp, attack_type, reason)
    ...
    def _start_return_to_start_override(self, sensor: str, timestamp: float,
        ...
        self._request_contingency(
            timestamp, ContingencyAction.HOLD,
            "Confirmed high-frequency GNSS integrity attack; hold for operator contingency"
        )
    ...
    def _start_directional_recovery(self, sensor: str, timestamp: float,
        ...
        self._request_contingency(
            timestamp, ContingencyAction.HOLD,
            "Confirmed directional GNSS integrity attack; hold for operator contingency"
        )
    ...
    def process_sensor_reading(self, reading: SensorReading, R_cov: np.ndarray, *args):
        ...
        if slow_sig.status.value >= 2 or ml_sig.status == MLStatus.ALARM:
            ...
            if saarm_sig.verdict == SaarmVerdict.ISOLATED or saarm_sig.verdict == SaarmVerdict.DEAD_RECKONING:
                ...
                if self.aircraft_mode:
                    self._request_contingency(
                        t, ContingencyAction.HOLD,
                        "GNSS source quarantined after integrity monitor alarm"
                    )
    ...
    def _trigger_hijack_response(self, timestamp: float, tampered_dest: np.ndarray, reason: str):
        ...
        # 1. Aircraft-safe hold request (or brake stop for non-aircraft users).
        self._protective_stop(timestamp, "WAYPOINT_HIJACK", reason)
